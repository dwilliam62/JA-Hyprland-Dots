...
icon_off() {
  ...
  # Nerd Font: use a simple sun icon that’s widely available
  # Using FA sun-o fallback char: \uf185 ()
...
icon_on() {
  case "$ICON_MODE" in
    sunset)
      # Try a sunset-like glyph; fallback to colored sun if font lacks glyph
      # Common Nerd Font weather sunset: some sets have , but not universal.
      # We’ll color the sun orange as a safe, widely-renderable choice.
      printf "<span foreground='orange'></span>"
      ;;
    blue)
      # Blue sun (user requested alternative)
